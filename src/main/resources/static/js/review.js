...
var vm = new Vue({
    ...
    methods: {
        ...
        search(pageCode, pageSize) {
            this.loadings();
            this.$http.post('/review/findByConPage?pageSize=' + pageSize + '&pageCode=' + pageCode, this.searchEntity).then(result => {
                console.log(result);
                this.review = result.body.rows;
        ...
        //清空已选择的
        clearSelect() {
            this.$refs.review.clearSelection();
        },
        //pageSize改变时触发的函数
        ...
        sureDelete(ids) {
            this.$confirm('你确定永久删除影评信息吗？', '提示', {
                ...
                this.$http.post('/review/delete', JSON.stringify(ids)).then(result => {
        ...
        deleteSelect(rows) {
            if (rows) {
                rows.forEach(row => {
                    ...
                    this.$refs.review.toggleRowSelection(row);
                });
                //调用删除方法
