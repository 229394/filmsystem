...
var vm = new Vue({
    ...
    methods: {
        ...
        search(pageCode, pageSize) {
            this.loadings();
            this.$http.post('/order/findByConPage?pageSize=' + pageSize + '&pageCode=' + pageCode, this.searchEntity).then(result => {
                console.log(result);
                this.order = result.body.rows;
        ...
        sureDelete(ids) {
            this.$confirm('你确定永久删除订单信息吗？', '提示', {
                ...
                this.$http.post('/order/delete', JSON.stringify(ids)).then(result => {
        ...
        deleteSelect(rows) {
            if (rows) {
                rows.forEach(row => {
                    ...
                    this.$refs.order.toggleRowSelection(row);
                });
                //调用删除方法
